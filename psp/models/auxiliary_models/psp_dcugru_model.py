...
def build_model(params):
    """
    Description:
        Building PSP-DCUGRU neural network model. 3x1DConv convolutional layers
        followed by 3 unidirectional GRU recurrent layers and 3 fully-connected layers.
    Args:
    ...
    gru_f1 = GRU(**{**params["gru"], **params["gru1"]})(conv_features)

    gru_f2 = GRU(**{**params["gru"], **params["gru2"]})(gru_f1)

    gru_f3 = GRU(**{**params["gru"], **params["gru3"]})(gru_f2)

    ############################################################################

    concat_features = Concatenate(axis=-1)([gru_f1, gru_f2, gru_f3, conv_features])
    concat_features = Dropout(**params["dropout4"])(concat_features)

    #Dense Fully-Connected DNN layers
    after_gru_dense = Dense(**params["dense1"])(concat_features)
    after_gru_dense_dropout = Dropout(**params["dropout5"])(after_gru_dense)

    after_gru_dense = Dense(**params["dense2"])(after_gru_dense_dropout)
    after_gru_dense_dropout = Dropout(**params["dropout6"])(after_gru_dense)

    #Final Dense layer with 8 nodes for the 8 output classifications
    main_output = Dense(**params["dense3"])(after_gru_dense_dropout)
