...
from tensorflow.keras.layers import Input, Conv1D, Embedding, Dense, Dropout, Concatenate, BatchNormalization
...
def build_model(params):
    """
    Description:
        Building PSP-CNN neural network model. 3x1DConv convolutional layers
        followed by 3 dense fully-connected layers.
    Args:
    ...
    concat_features = Dropout(**params["dropout3"])(conv_features)

    #Dense Fully-Connected DNN layers
    after_cnn_dense = Dense(**params["dense1"])(concat_features)
    after_cnn_dense_dropout = Dropout(**params["dropout4"])(after_cnn_dense)

    after_cnn_dense = Dense(**params["dense2"])(concat_features)
    after_cnn_dense_dropout = Dropout(**params["dropout5"])(after_cnn_dense)

    #Final Dense layer with 8 nodes for the 8 output classifications
    main_output = Dense(**params["dense3"])(after_cnn_dense_dropout)
