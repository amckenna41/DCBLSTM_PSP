...
#     conv2_features = Conv2D(84,3,strides=1,activation='relu', padding='same')(max_pool_2D)
    conv2_features = Conv2D(42,3,strides=1,activation='relu', padding='same', kernel_regularizer = "l2")(conv1_features)
    ...
    # ######## Recurrent Layers ########
    lstm_f1 = Bidirectional(LSTM(400,return_sequences=True,activation = 'tanh', recurrent_activation='sigmoid',dropout=0.5,recurrent_dropout=0.5))(conv2_features)
    ...
    protein_features_dropout = Dropout(0.4)(protein_features)
    # protein_features_2 = Dense(300, activation='relu')(protein_features)
    # protein_features_dropout = Dropout(0.2)(protein_features_2)
    # protein_features_3 = Dense(150, activation='relu')(protein_features_2)
    # protein_features_dropout = Dropout(0.2)(protein_features_3)
    ...
    checkpoint_path = "/lstm_2conv_" + str(datetime.date(datetime.now())) + ".h5"
...
def main(args):
    ...
    #build model
    print('Building 2conv BLSTM model')
    model = build_model()
...
#this will authenticate google account with google sdk, crdenetials will be automatically stored in config file which gcloud sdk looks for credentials
    ...
    model_save_path = 'model_blstm_3conv_' +'epochs_' + str(args.epochs) +'_'+ 'batch_size_' + str(args.batch_size) + '_' + str(datetime.date(datetime.now())) + \
